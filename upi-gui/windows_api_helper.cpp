...
WindowsAPIHelper::WindowsAPIHelper() : game(this) {

}

// ウィンドウ生成
bool WindowsAPIHelper::createWindow() {
	// windowクラス登録
	WNDCLASSEX wc;
	wc.cbSize = sizeof(WNDCLASSEX);
	wc.style = CS_HREDRAW | CS_VREDRAW;
	wc.lpfnWndProc = (WNDPROC)WindowsAPIHelper::windowProc;
	wc.cbClsExtra = 0;
	wc.cbWndExtra = 0;
	...
	// ウィンドウ生成、表示
	main_window_handle = CreateWindow(wc.lpszClassName, wc.lpszClassName,
...
bool WindowsAPIHelper::onCreate() {
	...
	// combobox
	child_window[COMBO_AI1P] = ChildWindow(TEXT("COMBOBOX"), NULL, style_combo | WS_DISABLED, 375, 550, 100, 600);
	child_window[COMBO_AI2P] = ChildWindow(TEXT("COMBOBOX"), NULL, style_combo | WS_DISABLED, 500, 550, 100, 600);

	// checkbox
	child_window[CHECK_AI1P] = ChildWindow(TEXT("BUTTON"), TEXT("AI"), style_check, 133, 510, 40, 30);
	child_window[CHECK_AI2P] = ChildWindow(TEXT("BUTTON"), TEXT("AI"), style_check, 500, 510, 40, 30);
	child_window[CHECK_CONTINUE_BATTLE] = ChildWindow(TEXT("BUTTON"), TEXT("Continuous"), style_check, 433, 460, 130, 30);
	child_window[CHECK_PLAY_SOUND] = ChildWindow(TEXT("BUTTON"), TEXT("Voice"), style_check, 433, 485, 130, 30);

	// menuは皮だけ作っておこう
	for (EventID eid = MENU_OPEN_PUYOFU; eid <= MENU_VERSION; eid = EventID(eid + 1)) {
...
bool WindowsAPIHelper::initEngineList() {
...
// 終了処理
void WindowsAPIHelper::onDestroy() {
	for (int p = 0; p < 2; p++) {
		for (int i = 0; i < 11; i++) {
...
void WindowsAPIHelper::playVoice(int player, int chain) const {
...
HDC WindowsAPIHelper::dcHandle(BitmapID bid) const {
    return mdc.at(bid).handle;
}

void WindowsAPIHelper::show(BitmapID bid) const {
    BitBlt(mdc.at(MEMORY).handle, 0, 0, WIN_WIDTH, WIN_HEIGHT, mdc.at(bid).handle, 0, 0, SRCCOPY);
}

// イベントハンドラを定義する。
void WindowsAPIHelper::setHandler() {
	...
	commandHandler(MENU_ADD_ENGINE) = [&](HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		if (GetOpenFileName(&ofn)) {
			if (engine_manager.addUPIEngine(*this, ofn.lpstrFile)) {
				initEngineList();
			}
		}
	};
...
void WindowsAPIHelper::onMCINotify(WPARAM wParam, LPARAM lParam) {
	if (wParam == MCI_NOTIFY_SUCCESSFUL) {
		mciSendCommand(lParam, MCI_SEEK, MCI_SEEK_TO_START, 0);
	}
}

LRESULT WindowsAPIHelper::onCtlColorStatic(WPARAM wParam, LPARAM lParam) {
	HBRUSH hbkgnd = NULL;
...
void WindowsAPIHelper::onTimer() {
	game.update();
}

void WindowsAPIHelper::gameOver() {
	if (!isChecked(CHECK_CONTINUE_BATTLE)) {
		KillTimer(main_window_handle, 100);
		close(B_STOP);
		enable(CHECK_AI1P);
...
void WindowsAPIHelper::drawString(RECT* rect, TCHAR* str, UINT format) {
...
void WindowsAPIHelper::rectClear(RECT *rect) {
    BitBlt(dcHandle(MEMORY), rect->left, rect->top, rect->right - rect->left, rect->bottom - rect->top, dcHandle(FIELD), rect->left, rect->top, SRCCOPY);
    InvalidateRect(main_window_handle, rect, false);
}
